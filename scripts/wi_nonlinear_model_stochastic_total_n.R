...
surface_b <- expand.grid(
  ...
  mutate(
    N_scaled = dplyr::case_when(
      N_label == "Low N (10th pct)" ~ N_quants[1],
      N_label == "Median N (50th pct)" ~ N_quants[2],
      N_label == "High N (90th pct)" ~ N_quants[3]
    ),
    yield_fitted = model_fn(OM_mean, N_scaled, fit_params["a0"], fit_params["a1"], fit_params["b0"], fit_params["b1"], fit_params["c0"], fit_params["c1"]),
...
combined <- grid.arrange(
  ...
  ncol = 2,
  top = grid::textGrob(
    "Wisconsin County Panel — Stochastic Total N Corn Yield Model (1995–2017)\nRandom split + randomized multi-start + bootstrap",
    gp = grid::gpar(fontsize = 13, fontface = "bold")
  )
)

ggsave(
  "./output/figures/figure_wi_model_visuals_stochastic_total_n.png",
  combined,
  width = 14,
  height = 10,
  dpi = 150,
...
write.csv(param_table, "./output/tables/table_wi_model_parameters_stochastic_total_n.csv", row.names = FALSE)
write.csv(metrics_table, "./output/tables/table_wi_model_metrics_stochastic_total_n.csv", row.names = FALSE)
